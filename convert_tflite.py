...
for fname in tflite_files:
    ...
    try:
        ...
        print(f"  Input:  {list(inp[0]['shape'])}")
        print(f"  Output: {list(out[0]['shape'])}")
        ops = set(d['op_name'] for d in interpreter._get_ops_details())
        print(f"  OPS ({len(ops)}): {sorted(ops)}")
        ...
        if flex:
            ...
            print(f"  All ops OK")
        dummy = np.random.rand(*inp[0]['shape']).astype(np.float32)
        interpreter.set_tensor(inp[0]['index'], dummy)
        interpreter.invoke()
        out_arr = interpreter.get_tensor(out[0]['index'])
